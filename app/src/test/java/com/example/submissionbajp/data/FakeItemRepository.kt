...
class FakeItemRepository constructor(
    ...
    override fun getFavoriteItem(): LiveData<PagedList<ItemsEntity>> {
        return LivePagedListBuilder(localDataSource.getFavoriteItemList(), 4).build()
    }


    override fun setFavorite(items: ItemsEntity, newState: Boolean) {
        appExecutors.diskIO().execute { localDataSource.setFavorite(items, newState) }
    }
}